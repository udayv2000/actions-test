...
def test_Edit_User_functionality_works_as_expected(guest_user):
    ...
    actual_edit_status = profile_page.edit_user(created_User_name)

    check.is_true(
        actual_edit_status is not False,
        f"Organization creation failed. Expected a Organization name, got: {actual_edit_status}"
    )
    print(f"➡️ Organization created successfully: '{actual_edit_status}'")
