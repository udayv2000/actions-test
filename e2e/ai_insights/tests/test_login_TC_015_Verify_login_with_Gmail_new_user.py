...
def test_Verify_login_with_Gmail_new_user(guest_user):
    home_page = Homepage(guest_user._page)
    login_page = LoginPage(guest_user._page)

    test_data = read_test_data(sheet_name="LoginPage", parent_tcid="TC_015", data_tcid="TC_015_01")

    expected_homepage_URL= test_data["HomePage_URL"].strip()
    expected_mail= test_data["Mail"].strip()
    expected_password= test_data["Password"].strip()

    # Click on login button
    home_page.open_login()  

    actual_latticeflow_url = login_page.verify_login_with_gmail(expected_mail,expected_password,expected_homepage_URL)
